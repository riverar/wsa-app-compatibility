...
| Honkai Impact 3rd | 7.3.0 | 11, 13 | ⚠️ | Graphics quality is sub-par, such as the anti-aliasing, which isn't as smooth for Kiana and Mei models even at "Max" graphics quality - [as shown in this gameplay video](https://youtu.be/6e-RQ2b2hoM). Combat works but during cutscenes, a black screen appears but cutscene audio still plays in the background. Dialog scenes still appear like normal. Cannot move the character using WASD controls but touch controls for movement/combat skill still works. | Tested under WSA with Google Play, under NVIDIA RTX 4060 Laptop GPU (Vulkan driver is D3D12, but not enabled). Recommended to change the graphic settings after installing the game for optimal performance.

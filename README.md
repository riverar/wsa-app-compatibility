...
| Google Services Framework | 12, API 32 | 12 | ❌ | Although installation succeeds and apps become aware of it, it lacks a lot of permissions needed for most functions, e.g. `read_device_config`, which can't be given even with a sideloaded Settings app.
...
| Settings | 12, API 32 | 12 | ⚠️ | Setting screenlock to Swipe, makes it impossible to use any apps without re-installing the entire Subsystem. Adding a Google account in the Account menu doesn't work. | Included by default in Subsystem. Accessed by creating a Windows shortcut with this path: `%LOCALAPPDATA%\Microsoft\WindowsApps\MicrosoftCorporationII.WindowsSubsystemForAndroid_8wekyb3d8bbwe\WsaClient.exe /launch wsa://com.android.settings`
